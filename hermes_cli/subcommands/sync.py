"""``hermes sync`` parser — bare fork sync plus upstream Skill Sync actions.
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        "sync",
        help="Sync the Hermes fork (bare) or skills across devices (subcommands)",
        description=(
            "With no subcommand, fetch and merge upstream into this Hermes fork, then push origin. "
        ...
        epilog=(
            "Examples:\n"
            "  hermes sync                   merge upstream into this fork and push origin\n"
            "  hermes sync status            what skills are synced, and from where\n"
