...
def _sync_profile_config_migrations() -> None:
    """Apply safe non-interactive config migrations to every Hermes profile."""
    try:
        ...
        for profile in profiles:
            try:
                result = migrate_profile_config(profile.path, quiet=True)
                changed = bool(
                    result
                    and (
                        result.get("env_added")
                        or result.get("config_added")
                        or result.get("version_updated")
                    )
                )
                print(f"  {profile.name}: {'updated' if changed else 'up to date'}")
            except Exception as exc:
                print(f"  {profile.name}: error ({exc})")
    except Exception as exc:
        logger.debug("Profile config migration sync failed: %s", exc)
