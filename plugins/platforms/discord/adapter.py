...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        if any(token in content for token in ("중단", "타임아웃", "장애")):
            emoji = "⚠️"
        elif any(token in content for token in ("버그", "오류", "에러", "실패")):
            emoji = "🐛"
        elif any(token in content for token in ("배포", "릴리즈", "출시", "런칭")):
            emoji = "🚀"
        elif any(token in content for token in ("스레드", "쓰레드", "제목")) or "thread" in lowered:
            emoji = "🧵"
        elif any(token in content for token in ("문서", "가이드", "정리", "요약")):
            emoji = "📝"
        elif any(token in content for token in ("설정", "구성", "인증서")) or any(
            token in lowered for token in ("config", "configuration", "ssl", "cert")
        ):
            emoji = "⚙️"
        elif any(token in content for token in ("보안", "권한", "인증", "인가")) or "oauth" in lowered:
            emoji = "🔐"
        else:
            emoji = "💬"
        body = content[:24].rstrip()
        if len(content) > 24:
            body = body[:23].rstrip() + "…"
        return f"{emoji} {body}"
